    ...
    t.prototype.init = function () {
        a('[data-plugin="dragula"]').each(function () {
            ...
            r ? dragula(n, {
                ...
                var sort = [];
                const position = $(target).children('.board-list-card').index(el) + 1;
                var id = $("#" + el.id).data('board-list-card-id');
                var board_list_id = $("#" + target.id).data('status');

                $("#" + source.id).parent().find('.count').text($("#" + source.id + " > div").length);
                $("#" + target.id).parent().find('.count').text($("#" + target.id + " > div").length);
                const loading_div = $('.loading_div');

                const url = base_url + 'board/board-card-change';
                const formData = new FormData();
                formData.append("id", id)
                formData.append("board_list_id", board_list_id)
...
//
//                 $.ajax({
//                     url: base_url+'board/board-card-change',
//                     type: 'GET',
//                     data: "id="+id+"&board_list_id="+board_list_id,
//                     dataType: "json",
