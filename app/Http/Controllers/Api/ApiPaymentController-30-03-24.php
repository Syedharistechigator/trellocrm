...
class ApiPaymentControllertest extends Controller
{
    ...
    private function card_available($card_number, $cvv, $request, $merchant_id = 0)
    {
        if ($merchant_id == 0) {
            return false;
        }
...
//            ->where('cc_infos.card_name', $request->card_name)
    ...
    private function response_array($response)
    {
        ...
        if (isset($response['payment_gateway']) && $response['payment_gateway'] === 'expigate') {
            $responseArray['payment_error'] = $responseArray['response']['responsetext'] ?? null;
        }
        ...
        if (isset($response['response']) && is_array($response['response'])) {
            ...
            return $this->failed_request($response, 500, $response['payment_gateway']);
        }
    ...
    private function payment_gateway($invoice, $request)
    {
        ...
        /** => Amazon Kindle Direct Publisher = 3 on Expigate Payment Method Start <= */
//        if payment not available on this card within 30 days and amount is less than 3000, payment should be on Amazon
//        if ($payment_complete === 0 && isset($invoice->getBrand) && $invoice->getBrand->is_amazon == 1 && $invoice->getBrand->merchant_id != 1) {
...
//            } else {
//                $card_available = $this->card_available($card_number_enc, $cvv_enc, $request, 3);
//                if ($card_available && ($invoice->total_amount > 0 && $invoice->total_amount <= 3000)) {
//                    $api_url = route('api.expigate.payment');
//                    $inputs['payment_gateway'] = 'amazon';
//                    $inputs['amazon_merchant_id'] = 3;
//                    $payment_gateway = 2;
//                    $response = Http::post($api_url, $inputs);
//                    $IS_AMAZON = true;
...
//                    }
//                } else {
//                    $payment_process_from['amazon']['Kindle_Direct_Publishers'] = ['error' => 'Card already exists. 3'];
//                }
//            }
//        }
        /** => Amazon Kindle Direct Publisher = 3 on Expigate Payment Method End <= */

        /** => Amazon KDP Publisher = 4 on Expigate Payment Method Start <= */
        // If payment not available on this card within 30 days and amount is less than 3000, payment should be on Amazon
//
        if ($payment_complete === 0 && isset($invoice->getBrand) && $invoice->getBrand->is_amazon == 1 && $invoice->getBrand->merchant_id != 1) {
            $expigate_rules = ['zipcode' => 'required', 'city' => 'required', 'state' => 'required'];
            ...
            if ($expigate_validator->fails()) {
                ...
                if ($VISA_CARD) {
                    $card_available = $this->card_available($card_number_enc, $cvv_enc, $request, 4);
                    if ($card_available && ($invoice->total_amount > 0 && $invoice->total_amount <= 3000)) {
                        $api_url = route('api.expigate.payment');
                        $inputs['payment_gateway'] = 'amazon';
                        $inputs['amazon_merchant_id'] = 4;
                        $payment_gateway = 2;
                        $response = Http::post($api_url, $inputs);
                        $IS_AMAZON = true;
                        ...
                        $payment_process_from['amazon']['KDP_Publishers'] = ['error' => 'Card already exists. 4'];
                    }
                } else {
        ...
        /** => Amazon KDP Publisher = 4 on Expigate Payment Method End <= */
...
//         If payment not available on this card within 30 days and amount is less than 3000, payment should be on expigate
        if ($payment_complete === 0 && isset($invoice->getBrand) && $invoice->getBrand->merchant_id != 1) {
            $expigate_rules = ['zipcode' => 'required', 'city' => 'required', 'state' => 'required'];
            $expigate_validator = Validator::make($request->all(), $expigate_rules);
            if ($expigate_validator->fails()) {
                $errors = $expigate_validator->errors()->toArray();
                ...
                if ($IS_ERROR === false) {
                    if (isset($invoice->getBrand->getMerchantExpigate) && $invoice->getBrand->getMerchantExpigate->name === 'Expigate') {
                        $card_available = $this->card_available($card_number_enc, $cvv_enc, $request, $invoice->getBrand->expigate_id);
                        if ($card_available && ($invoice->total_amount > 0 && $invoice->total_amount <= 3000)) {
                            $api_url = route('api.expigate.payment');
                            ...
                            if ($response->json() != null) {
                                if ($response->json()['response'] && $response->json()['response']['response_code'] != '100') {
                                    $payment_process_from['expigate'] = [$response->json()];
                                } elseif ($response->failed() && $response->json()['response'] && $response->json()['response']['response_code'] != '100') {
                                    $failed_response = json_decode($this->failed_request($response->json(), 500, 'expigate')->getcontent(), true);
                                    $payment_process_from['expigate'] = [$failed_response['errors']];
                                } elseif ($response->json()['response'] && $response->json()['response']['response_code'] == '100') {
                                    $payment_complete = 1;
                                    $payment_process_from['expigate'] = [$response->json()];
                                }
                            ...
                            $payment_process_from['expigate'] = ['error' => 'Card already exists. ' . $invoice->getBrand->expigate_id];
                        }
                    } else {
                        $payment_process_from['expigate'] = ['error' => 'Payment method is not Expigate.'];
                    }
                } else if ($IS_ERROR === true && $ERROR) {
                    $payment_process_from['expigate'] = ['error' => $ERROR];
                }
            }
        }
        /** => Expigate Payment Method End <= */

        /** => Authorize Payment Method Start */

        /** => Authorize Payment Method <= Get Writing Service*/
        if (!$IS_AMAZON) {

            if ($payment_complete === 0 && isset($invoice->getBrand) && $invoice->getBrand->merchant_id == 2 && $invoice->total_amount <= 5000
//            && $invoice->team_key !=770877 /** get writing disabled to allan*/
            ) {
                $inputs['payment_gateway'] = 'authorize';
                $payment_gateway = 1;
                $inputs['merchant_id'] = 6;
                $response = Http::post(route('api.authorize.payment'), $inputs);
                if ($response->json() != null) {
                    if (!$response->failed() && $response->json()['resultCode'] == "Ok" && $response->json()['t_resp']['responseCode'] == 1) {
                        $payment_complete = 1;
                        $payment_process_from['authorize'][str_replace(' ', '_', 'Get Writing Service')] = [$response->json()];
                    } elseif ($response->failed()) {
                        $failed_response = json_decode($this->failed_request($response->json(), 500, 'authorize')->getcontent(), true);
                        $payment_process_from['authorize'][str_replace(' ', '_', 'Get Writing Service')] = [$failed_response['errors']];
            ...
            } /** => Authorize Payment Method <= Only for Fresh Payment  === Get Writing Service**/
            else if (!$IS_AMAZON && $invoice->sales_type == 'Fresh' && $payment_complete === 0 && isset($invoice->getBrand) && $invoice->getBrand->merchant_id == 2
//            && $invoice->team_key !=770877 /** get writing disabled to allan*/
            ) {
                $inputs['payment_gateway'] = 'authorize';
                $payment_gateway = 1;
            ...
            $payment_process_from['authorize'][str_replace(' ', '_', 'Get Writing Service')] = ['error' => $ERROR_MSG];
        }
        if ($payment_complete === 0) {
            /** Uncomment to block easy writing merchant on authorize */
            if (isset($invoice->getBrand) && $invoice->getBrand->merchant_id != 2) {
                $inputs['payment_gateway'] = 'authorize';
                $payment_gateway = 1;
                $inputs['merchant_id'] = null;
                $response = Http::post(route('api.authorize.payment'), $inputs);
                if ($response->json() != null) {
                    if (!$response->failed()) {
                        $payment_complete = 1;
                        $payment_process_from['authorize'][str_replace(' ', '_', $merchant_name)] = [$response->json()];
                    } elseif ($response->failed()) {
                        $failed_response = json_decode($this->failed_request($response->json(), 500, 'authorize')->getcontent(), true);
                        $payment_process_from['authorize'][str_replace(' ', '_', $merchant_name)] = [$failed_response['errors']];
                    } elseif ($response->json()['resp'] == null || $response->json()['t_resp'] == null || $response->json()['resultCode'] != "Ok" || $response->json()['t_resp']['responseCode'] != 1) {
                        $payment_process_from['authorize'][str_replace(' ', '_', $merchant_name)] = [$response->json()];
                    }
                } else {
                ...
                $payment_process_from['authorize'][str_replace(' ', '_', $merchant_name)] = ['error' => 'Easy Writing is temp disabled'];
            }
        }

        /** => Authorize Payment Method End */
        ...
        $multi_payment_response = new MultiPaymentResponse();
        $multi_payment_response->invoice_id = $inputs['invoice_id'];
        $multi_payment_response->response = json_encode($response && $response->json() ? $response->json() : null);
        $multi_payment_response->payment_gateway = $inputs['payment_gateway'];
        $multi_payment_response->payment_process_from = json_encode($payment_process_from);
        $multi_payment_response->response_status = $response && $response->status() ? $response->status() : 400;
        $multi_payment_response->form_inputs = json_encode($form_inputs);
        $multi_payment_response->save();

        return response()->json([
            'response' => $response && $response->json() ? $response->json() : "",
            'payment_gateway' => $inputs['payment_gateway'],
            'payment_process_from' => $payment_process_from
