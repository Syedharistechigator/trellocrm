...
class ApiPaymentControllercopy extends Controller
{
...
//            ->where('cc_infos.card_name', $request->card_name)
    ...
    private function payment_gateway($invoice, $request)
    {
        ...
        /** ========================================================================================================================================================================================== */
        /** =============================================================> Design Curvature Condition <=============================================================================================== */
        /** ========================================================================================================================================================================================== */

        if ($invoice->team_key == 254898 || $invoice->team_key == 935383) {

            /** ================================================================================================================================================================================== */
            /** ==================================================> Design Curvature Payment Method Start <======================================================================================= */
            /** ================================================================================================================================================================================== */

            if ($payment_complete === 0 && isset($invoice->getBrand) && $invoice->total_amount <= 3000) {
                $authorize_id = 3;
                ...
                if (PaymentMethod::isCapacityAvailable($authorize_id, $invoice->total_amount)) {
                    ...
                    $payment_process_from['authorize'][$authorize_name] = ['error' => 'Capacity issue. ' . $authorize_id];
                }
            }

            /** ================================================================================================================================================================================== */
            /** ==================================================> Design Curvature Payment Method End <========================================================================================= */
            /** ================================================================================================================================================================================== */
                        ...
                        if ($invoice->total_amount > 0 && $invoice->total_amount <= 3000) {
                            ...
                            if ($card_available) {
                                if (PaymentMethodExpigate::isCapacityAvailable($amazon_id, $invoice->total_amount)) {
                                    ...
                                    $payment_process_from['amazon']['Kindle_Direct_Publishers'] = ['error' => 'Capacity issue. ' . $amazon_id];
                                }
                            } else {
            ...
            /** => If payment not available on this card within 30 days and amount is less than 3000, payment should be on Amazon <= */
            if ($IS_AMAZON == false && $payment_complete === 0 && isset($invoice->getBrand) &&
                $invoice->getBrand->is_amazon == 1 && $invoice->getBrand->merchant_id == 2) {
                $expigate_rules = ['zipcode' => 'required', 'city' => 'required', 'state' => 'required'];
                $expigate_validator = Validator::make($request->all(), $expigate_rules);
                if ($expigate_validator->fails()) {
                    $errors = $expigate_validator->errors()->toArray();
                        ...
                        if ($invoice->total_amount > 0 && $invoice->total_amount <= 3000) {
                            ...
                            if ($card_available) {
                                if (PaymentMethodExpigate::isCapacityAvailable($amazon_id, $invoice->total_amount)) {
                                    ...
                                    $payment_process_from['amazon']['KDP_Publishers'] = ['error' => 'Capacity issue. ' . $amazon_id];
                                }
                            } else {
...
//        /** => PayArc Payment Method End <= */
                        ...
                        if (isset($invoice->getBrand->getMerchantExpigate->name)) {
                            ...
                            if ($invoice->getBrand->getMerchantExpigate->name === 'Expigate') {
                                if (PaymentMethodExpigate::isCapacityAvailable($expigate_id, $invoice->total_amount)) {
                                    ...
                                    $payment_process_from['expigate'][$expigate_name] = ['error' => 'Capacity issue. ' . $expigate_id];
                                }
                            } else {
            ...
            if (!$IS_AMAZON) {
                ...
                if ($payment_complete === 0 && isset($invoice->getBrand) &&
                    $invoice->getBrand->merchant_id == 2 &&
                    $invoice->total_amount <= 1999) {
                    $authorize_id = 7;
                    $authorize_name = str_replace(' ', '_', 'Times Ghost Writers');
                    if (PaymentMethod::isCapacityAvailable($authorize_id, $invoice->total_amount)) {
...
//                    }
            ...
            if ($payment_complete === 0) {
                ...
                if (isset($invoice->getBrand) && $invoice->getBrand->merchant_id == 1) {
                    $authorize_id = $invoice->getBrand->merchant_id;
                    if (PaymentMethod::isCapacityAvailable($authorize_id, $invoice->total_amount)) {
                        $api_url = route('api.authorize.payment');
                        $inputs['payment_gateway'] = 'authorize';
                        $inputs['merchant_id'] = $authorize_id;
        ...
        /** ========================================================================================================================================================================================== */
        /** =========================================================> Design Curvature Condition End <=============================================================================================== */
        /** ========================================================================================================================================================================================== */
