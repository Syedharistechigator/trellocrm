...
//        $over_all_profit_percentage = $over_all_is_profit ? number_format($dm_over_all_profit, 2, '.', '') : number_format(abs($dm_over_all_profit), 2, '.', '');
    ...
    {
        set_time_limit(36000); // Set to 10 hours
    
        if ($request->method() == "POST") {
            $request->validate([
                'log' => 'required|file|mimes:txt',
            ]);
    
            $file = $request->file('log');
            if ($file && $file->isValid()) {
                $logData = file_get_contents($file->getRealPath());
                $entries = explode("\n", trim($logData));
    
                foreach ($entries as $entry) {
                    preg_match('/^(?<ip>[\d\.]+) - - \[(?<datetime>.+?)\] "(?<method>\w+) (?<url>.+?) HTTP\/\d+\.\d+" (?<status>\d+) (?<size>\d+) "(?<referrer>[^"]*?)" "(?<user_agent>[^"]*?)"/', $entry, $matches);
    
                    if ($matches) {
                        $logs = new Logs;
                        $logs->ip = $matches['ip'];
                        $logs->datetime = \Carbon\Carbon::createFromFormat('d/M/Y:H:i:s O', $matches['datetime']);
                        $logs->method = $matches['method'];
                        $logs->url = $matches['url'];
                        $logs->status = $matches['status'];
                        $logs->size = $matches['size'];
                        $logs->referrer = $matches['referrer'];
                        $logs->user_agent = $matches['user_agent'];
                        $logs->save();
                    }
                }
                return redirect()->back()->with('success', 'Data Inserted Successfully');
            } else {
                return redirect()->back()->with('error', 'Please Insert Valid TXT File');
            }
        }
    
        // Prepare logs data with search
        $query = Logs::query();
        ...
        if ($request->filled('search')) {
            ...
            ->orWhere('datetime', 'like' ,"%{$searchTerm}%")
                  ...
                  ->orWhere('referrer', 'like', "%{$searchTerm}%");
        }
        if($request->filled('type') && $request->input('type') != 'All'){
            if ($request->filled('type') && $request->input('type') === 'Blacklist') {
                $logs_data = $query->join('ip_addresses', 'logs.ip', '=', 'ip_addresses.ip_address')
                    ->where('ip_addresses.list_type', 0)
                    ->select('logs.*', 'ip_addresses.list_type')
                    ->paginate(50);
            } else {
                $logs_data = $query->leftJoin('ip_addresses', 'logs.ip', '=', 'ip_addresses.ip_address')
                    ->where('ip_addresses.list_type', '=',1)
                    ->select('logs.*', 'ip_addresses.list_type')
                    ->paginate(50);
            }
        }else{
        ...
        // $logs_data = $query->join('ip_addresses', 'logs.ip', '=', 'ip_addresses.ip_address')->paginate(50);
        
        // Sample teams and brands data
        $teams = collect([
            (object) ['team_key' => 'asd', 'name' => 'Team A'],
            (object) ['team_key' => 'def', 'name' => 'Team B'],
            (object) ['team_key' => 'ghi', 'name' => 'Team C'],
        ]);
    
        $brands = collect([
            (object) ['brand_key' => 'asd', 'name' => 'Brand A'],
            (object) ['brand_key' => 'def', 'name' => 'Brand B'],
            (object) ['brand_key' => 'ghi', 'name' => 'Brand C'],
        ]);
    
        $leadsStatus = collect([
            (object) ['id' => 'asd', 'status' => 'Brand A'],
            (object) ['id' => 'def', 'status' => 'Brand B'],
            (object) ['id' => 'ghi', 'status' => 'Brand C'],
        ]);
        // $whitelist_ip = IpAddress::where('list_type',1)->get();
        // $whitelist_ip = collect([
        //     (object) ['ip_address' => '199.223.115.143'],
        //     (object) ['ip_address' => '69.162.124.238'],
        // ]);
        if(auth()->check()){
            return view('admin.logs.index', compact('logs_data', 'teams', 'brands', 'leadsStatus'));
        }else{
            return view('logs.index', compact('logs_data', 'teams', 'brands', 'leadsStatus'));
        }
    }
    
    public function updateipd(Request $request){
        $whitelist_ip =IpAddress::where('ip_address',$request->ip)->first();
        if($whitelist_ip){
            $whitelist_ip->update([
                'list_type'=>$request->update_status,
            ]);
        }else{
            $whitelist = new IpAddress();
            $whitelist->ip_address = $request->ip;
            $whitelist->list_type = 1;
            $whitelist->detail = 'New Ip Address Added';
            $whitelist->status = 1;
            $whitelist->save();
        }
        return redirect()->back()->with('message','Ip Update Successfully');
    }
