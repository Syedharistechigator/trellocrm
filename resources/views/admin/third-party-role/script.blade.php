<script>
    ...
    $(document).ready(function () {
        ...
        if (TableId) {
            ...
            } else if (TableId === "RecordTrashedTable") {
        ...
        function createRowHtml(data) {
            ...
            return `<tr id="tr-${data.id}">
                        ...
                        <td class="align-middle text-nowrap">${data.formatted_created_at??''}</td>
                        ...
                        <td class="align-middle text-nowrap">
                            <button data-id="${data.id}" title="Edit" class="btn btn-warning btn-sm btn-round editThirdPartyRole" data-toggle="modal" data-target="#editThirdPartyRoleModal">
        ...
        $('form').on('submit', function (e) {
            e.preventDefault();
            let url = $(this).attr('id') === "update_form" ? '{{ route('admin.third.party.role.update','/')}}/' + $('#editThirdPartyRoleModal').attr('data-id') : ($(this).attr('id') === "create_form" ? '{{ route('admin.third.party.role.store') }}' : null);
        ...
        $('#team_key , #client_id').on('change', function () {
            let team_key = $('#team_key').val() ? $('#team_key').val() : 0;
            let client_id = $('#client_id').val() ? $('#client_id').val() : 0;
            let url = $(this).attr('id') === "team_key" ? '{{ route('admin.third.party.role.team.agents.clients','/')}}/' + team_key : ($(this).attr('id') === "client_id" ? '{{ route('admin.third.party.client.paid.invoices', '/') }}/' + team_key + '/' + client_id : null);
            $('#invoice_id').empty().selectpicker('refresh').append(`<option class="" value="" disabled>Select Invoice</option>`);

            AjaxRequestGetPromise(url, null, null, false, '{{route("admin.third.party.role.index")}}', false, true, false).then((res) => {
        ...
        $('#edit_team_key , #edit_client_id').on('change', function () {
            ...
            let url = $(this).attr('id') === "edit_team_key" ? '{{ route('admin.third.party.role.team.agents.clients','/')}}/' + team_key : ($(this).attr('id') === "edit_client_id" ? '{{ route('admin.third.party.client.paid.invoices', '/') }}/' + team_key + '/' + client_id : null);
        ...
        $(document).on('click', '.editThirdPartyRole', function (e) {
            ...
            let url = '{{ route('admin.third.party.role.edit','/')}}/' + $(this).attr('data-id');
        ...
        /** Delete*/
        $("#ThirdPartyRoleTable").on("click", ".delButton", function () {
            swal({
                title: "Are you sure?",
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var id = $(this).data("id");
                        var url = '{{ route('admin.third.party.role.destroy','/')}}/' + id;

                        var res = AjaxRequestGet(url, null, 'Poof! Your record has been deleted!', false, null);
                        if (res && res.success) {
                            $('#ThirdPartyRoleTable').DataTable().row($("#tr-" + id)).remove().draw(false);
                        }
                    } else {
        ...
        $("#RecordTrashedTable").on("click", ".restoreButton", function () {
            ...
            var url = '{{ route('admin.third.party.role.restore','/')}}/' + restoreId;
        ...
        $("#restoreAll").on("click", ".restoreAllButton", function () {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var res = AjaxRequestGet('{{ route('admin.third.party.role.restore.all')}}', null, 'Restore all records successfully!', false, null);
                        if (res && res.success) {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        var url = '{{ route('admin.third.party.role.force.delete','/')}}/' + delId;
                        var res = AjaxRequestGet(url, {'third_party_role_id': delId}, 'Record permanent deleted successfully!', false, null);
                        if (res && res.success) {
