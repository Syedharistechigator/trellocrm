@extends('admin.layouts.app')@section('cxmTitle', 'Carry Forward')
@section('content')
    @push('css')
        @include('admin.team.carry-forward.style')
    @endpush
                    ...
                    <div class="col-lg-7 col-md-6 col-sm-12">
                        ...
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="zmdi zmdi-home"></i> {{ config('app.home_name') }}
                                </a></li> <li class="breadcrumb-item"> Carry Forwards </li>
                            <li class="breadcrumb-item active"><a href="{{route('admin.team.carry-forward.index')}}"> List</a></li>
                        </ul>
                        <button class="btn btn-primary btn-icon mobile_menu" type="button">
                            <i class="zmdi zmdi-sort-amount-desc"></i></button>
                    </div>
                    <div class="col-lg-5 col-md-6 col-sm-12 text-right">
                        <button class="btn btn-success btn-icon rounded-circle" type="button" data-toggle="modal" id="create-carry-forward-modal-btn" data-target="#createCarryForwardModal">
                    ...
                    <div class="col-lg-12">
                        <div class="">
                            <div class="table-responsive">
                                <table id="CarryForwardTable" class="table table-striped table-hover xjs-basic-example theme-color">
                                    <thead>
                                    <tr>
                                    ...
                                    @foreach($carry_forwards as $carry_forward)
                                        <tr id="tr-{{$carry_forward->id}}">
                                            <td class="align-middle">{{$carry_forward->id}}</td>
                                            <td class="align-middle">{{optional($carry_forward->getTeam)->name}}</td>
                                            <td class="align-middle">${{$carry_forward->amount}}</td>
                                            <td class="align-middle">{{ DateTime::createFromFormat('!m', $carry_forward->month)->format('F') }}</td>
                                            <td class="align-middle">{{ $carry_forward->year }}</td>
                                            <td class="align-middle text-nowrap">
                                                <button data-id="{{$carry_forward->id}}" title="Edit" class="btn btn-warning btn-sm btn-round editCarryForward" data-toggle="modal" data-target="#editCarryForwardModal">
                                                    <i class="zmdi zmdi-edit"></i></button>
                                                <a title="Delete" data-id="{{$carry_forward->id}}" data-type="confirm" href="javascript:void(0);" class="btn btn-danger btn-sm btn-round delButton"><i class="zmdi zmdi-delete"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
    ...
    <div class="modal fade" id="createCarryForwardModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="title" id="defaultModalLabel">Create A New Carry Forward</h4>
                </div>
    ...
    <!-- Edit Carry Forward -->
    <div class="modal fade" id="editCarryForwardModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="title" id="defaultModalLabel">Edit Carry Forward</h4>
                </div>
...
@endsection
@push('cxmScripts')
    @include('admin.team.carry-forward.script')
@endpush
