<script>
    ...
    function getParam() {
        ...
        window.location.href = params.length > 0 ? "{{ route('admin.team.indirect-costing.index') }}?" + params.join('&') : "{{ route('admin.team.indirect-costing.index') }}";
    }
    ...
    $(document).ready(function () {
        ...
        if (TableId) {
            if (TableId === "IndirectCostingTable") {
                $('#IndirectCostingTable').DataTable().destroy();
                $('#IndirectCostingTable').DataTable({
                    dom: 'lBfrtip',
                    buttons: [
        ...
        $('#create-indirect-costing-modal-btn, .editIndirectCosting').click(function () {
            $('#search-team, #search-month, #search-year ,[type=search],select[name="IndirectCostingTable_length"]').prop('disabled', true);
        });
        $('#createIndirectCostingModal, #editIndirectCostingModal').on('hidden.bs.modal', function () {
            $('#search-team, #search-month, #search-year ,[type=search],select[name="IndirectCostingTable_length"]').prop('disabled', false);
        });
        $('#create-indirect-costing-modal-btn').on('click', function () {
            $('#create_form')[0].reset();
        ...
        function createRowHtml(data) {
            return `<tr id="tr-${data.id}">
                ...
                <td class="align-middle text-nowrap">
                    <button data-id="${data.id}" title="Edit" class="btn btn-warning btn-sm btn-round editIndirectCosting" data-toggle="modal" data-target="#editIndirectCostingModal">
        ...
        $('form').on('submit', function (e) {
            e.preventDefault();
            let url = $(this).attr('id') === "update_form" ? '{{ route('admin.team.indirect-costing.update','/')}}/' + $('#editIndirectCostingModal').attr('data-id') : ($(this).attr('id') === "create_form" ? '{{ route('admin.team.indirect-costing.store') }}' : null);
            if (url === null || ($(this).attr('id') === "update_form" && $('#editIndirectCostingModal').attr('data-id') === '')) {
                return false;
            }
            AjaxRequestPostPromise(url, new FormData(this), null, false, null, false, true, true).then((res) => {
                if (res.success && res.status && res.status === 1) {
                    ...
                    let table = $('#IndirectCostingTable').DataTable();
                    if (table) {
                ...
                console.log(error);
                if (error.status == 422 && error.responseJSON.errors && error.responseJSON.errors.year && error.responseJSON.errors.year[0] === "Indirect Costing for this team already exists for the specified month and year.") {
                    var table = $('#IndirectCostingTable').DataTable();
        ...
        $(document).on('click', '.editIndirectCosting', function (e) {
            e.preventDefault();
            $('#update_form')[0].reset();
            ...
            let url = '{{ route('admin.team.indirect-costing.edit','/')}}/' + $(this).attr('data-id');

            AjaxRequestGetPromise(url, null, null, false, '{{route("admin.team.indirect-costing.index")}}', false, true, false).then((res) => {
                if (res.status && res.status === 1 && res.success) {
                    $("#editIndirectCostingModal").attr('data-id', res.data.id);
                    $('#edit_team_key').val(res.data.team_key ? res.data.team_key : "").selectpicker('refresh');
                    $('#edit_amount').val(res.data.amount ? res.data.amount : "");
                    $('#edit_month').val(res.data.month ? res.data.month : "").selectpicker('refresh');
                    $('#edit_year').val(res.data.year ? res.data.year : "").selectpicker('refresh');
        ...
        /** Delete*/
        $("#IndirectCostingTable").on("click", ".delButton", function () {
            swal({
                title: "Are you sure?",
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        var id = $(this).data("id");
                        var url = '{{ route('admin.team.indirect-costing.destroy','/')}}/' + id;

                        var res = AjaxRequestGet(url, null, 'Poof! Your record has been deleted!', false, null);
                        if (res && res.success) {
                            $('#IndirectCostingTable').DataTable().row($("#tr-" + id)).remove().draw(false);
                        }
                    } else {
