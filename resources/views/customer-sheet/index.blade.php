@extends('layouts.app')@section('cxmTitle', 'Customer Sheet')
                    ...
                    <div class="col-lg-12">
                        <div class="">
                            <div class="table-responsive">
                                <table id="CustomerSheetTable" class="table table-striped table-hover xjs-basic-example theme-color">
                                    <thead>
                                    <tr>
                                        <th class='text-nowrap'>ID #</th>
                                        <th class='text-nowrap'>{{Auth::user()->type != 'tm-client' ? "Customer Id" : " Form Id"}}</th>
                                        <th class='text-nowrap'>{{Auth::user()->type != 'tm-client' ? "Customer Name" : "Name"}}</th>
                                        <th class='text-nowrap'>{{Auth::user()->type != 'tm-client' ? "Customer Email" : "Email"}}</th>
                                        <th class='text-nowrap'>{{Auth::user()->type != 'tm-client' ? "Customer Phone" : "Phone"}}</th>
                                        ...
                                        <tr id="tr-{{$customer_sheet->id}}">
                                            ...
                                            <td class="align-middle text-nowrap" id="action-btn-{{$customer_sheet->id}}">
                                                @if($customer_sheet->attachments() !== null && $customer_sheet->attachments()->count() > 0)
                                                    <button data-id="{{$customer_sheet->id}}" id="attachment-id-{{$customer_sheet->id}}" title="View Attachment" class="btn btn-info btn-sm btn-round viewAttachments" data-toggle="modal" data-target="#attachmentsModal">
                                                        <i class="zmdi zmdi-attachment-alt"></i></button>
                                                @endif
                                                @if(Auth::user()->type == 'tm-user')
                                                    <button data-id="{{$customer_sheet->id}}" id="add-attachment-id-{{$customer_sheet->id}}" title="Add Attachment" class="btn btn-info btn-sm btn-round addAttachments" data-toggle="modal" data-target="#addAttachmentsModal">
                                                        <i class="zmdi zmdi-plus-circle"></i></button>
                                                    <button data-id="{{$customer_sheet->id}}" title="Edit" class="btn btn-info btn-sm btn-round editCustomerSheet" data-toggle="modal" data-target="#editCustomerSheetModal">
                                                        <i class="zmdi zmdi-edit"></i></button>
                                                    <a title="Delete" data-id="{{$customer_sheet->id}}" data-type="confirm" href="javascript:void(0);" class="btn btn-danger btn-sm btn-round delButton" style="color: white;"><i class="zmdi zmdi-delete"></i></a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="title" id="defaultModalLabel">{{Auth::user()->type != 'tm-client' ? "Create A New Customer Sheet" : "Submit a new request"}}</h4>
                    </div>
                    <form method="POST" id="create-customer-sheet-form" enctype="multipart/form-data">
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="{{ $prefix }}name">Enter {{$labelPrefix}}Name</label>
                                <input type="text" id="{{ $prefix }}name" class="form-control" placeholder="Enter {{$labelPrefix}}Name" name="{{ $prefix }}name" required autocomplete="{{ $prefix }}name">
                            </div>
                            <div class="form-group">
                                <label for="{{ $prefix }}email">Enter {{$labelPrefix}}Email</label>
                                <input type="email" id="{{ $prefix }}email" class="form-control" placeholder="Enter {{$labelPrefix}}Email" name="{{ $prefix }}email" required autocomplete="{{ $prefix }}email">
                            </div>
                            <div class="form-group">
                                <label for="{{ $prefix }}phone">Enter {{$labelPrefix}}Phone Number</label>
                                <input type="text" id="{{ $prefix }}phone" class="form-control" placeholder="Enter {{$labelPrefix}}Phone Number" name="{{ $prefix }}phone" required autocomplete="{{ $prefix }}phone">
                            </div>
                            <div class="form-group">
                            ...
                            @endif
                            <div class="form-group">
                                <label for="cs-attachments">Upload Attachments (Optional)</label>
                                <input type="file" id="cs-attachments" class="form-control" name="attachments[]" multiple>
                            </div>
                        </div>
